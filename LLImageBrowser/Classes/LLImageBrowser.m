...
+(instancetype)showImages:(NSArray*)images imageTyep:(ImagesType)type placeholderImage:(UIImage *)image selectedIndex:(NSInteger)index fromView:(UIView *)fromView{
    LLImageBrowser *photoBrowser = [[LLImageBrowser alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
-(void)btnAction:(UIButton*)btn{
    NSString* url = self.images[self.currentPage];
    [LLImageTool saveImageWithURLString:url];
}
...
/// 统一动画
-(void)animateWithDuration:(CGFloat)duration animations:(void(^)(void))animations completion:(void(^)(void))completion notification:(BOOL)need{
